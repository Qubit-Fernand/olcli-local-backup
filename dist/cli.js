...
import { loadIgnore, shouldIgnore, buildTexSiblingSet, } from './ignore.js';
// Read version from package.json
const __dirname = dirname(fileURLToPath(import.meta.url));
const pkg = JSON.parse(readFileSync(join(__dirname, '..', 'package.json'), 'utf-8'));
const VERSION = pkg.version;
import { getSessionCookie, setSessionCookie, setLastProject, getConfigPath, saveOlAuth, clearConfig, getBaseUrl, setBaseUrl, getSessionCookieName, setSessionCookieName, getTimeout, setTimeout, getPasswordCredentials, setPasswordCredentials } from './config.js';
 ...
 */
async function getClient(cookieOpt, baseUrlOpt) {
    const baseUrl = baseUrlOpt || program.opts().baseUrl || getBaseUrl();
    const cookieName = program.opts().cookieName || getSessionCookieName();
    ...
    if (cookie) {
        try {
            ...
            const timeout = program.opts().timeout || getTimeout();
...
async function loginWithSavedPassword(credentials, baseUrl, cookieName) {
...
function persistClientSession(client, preferredCookieName) {
...
async function resolveProject(client, projectArg, dir = '.') {
...
program
    ...
    try {
        const baseUrl = program.opts().baseUrl || getBaseUrl();
        const cookieName = program.opts().cookieName || getSessionCookieName();
        if (options.cookie) {
...
function printCommentContext(comment) {
    if (!comment.context)
        return;
...
program
    ...
    try {
        ...
        const filesToUpload = [];
        const filesIgnored = [];
        function scanDir(currentDir, relativeBase = '') {
            const entries = readdirSync(currentDir, { withFileTypes: true });
            // Pre-compute sibling .tex set for the PDF special rule.
            const texSiblings = buildTexSiblingSet(entries.filter((e) => !e.isDirectory()).map((e) => e.name));
            for (const entry of entries) {
...
program
    ...
    try {
        ...
        const filesIgnored = [];
        const { readdirSync, statSync } = await import('node:fs');
        function scanLocalFiles(currentDir, relativeBase = '') {
        ...
        if (existsSync(metaPath)) {
            ...
            if (Array.isArray(meta.remoteManifest)) {
                previousManifest = meta.remoteManifest;
            }
        }
        const filesToUpload = [];
        const filesUpdatedLocally = [];
        const filesKeptLocal = [];
        const filesNewLocal = [];
        const filesDeletedRemote = [];
        const filesDeleteSkipped = [];
        ...
        const newManifest = new Set(remoteFiles.keys());
        for (const f of filesToUpload)
            newManifest.add(f.path);
        for (const p of filesDeletedRemote)
            newManifest.delete(p);
        // Update metadata
        if (!options.dryRun) {
