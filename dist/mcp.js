...
import { getSessionCookie, getBaseUrl, getSessionCookieName, setSessionCookie, setSessionCookieName, getPasswordCredentials, } from './config.js';
// ---------------------------------------------------------------------------
// Auth helpers
// ---------------------------------------------------------------------------
/**
...
function resolveSessionCookie() {
    ...
    if (stored)
        return stored;
    throw new Error('No Overleaf session cookie found.\n' +
...
// Tool: list_projects
// ---------------------------------------------------------------------------
server.tool('list_projects', 'List all Overleaf projects in the account. Returns project IDs, names, and metadata.', async () => wrapTool(async () => {
...
// ---------------------------------------------------------------------------
server.tool('get_project_info', 'Get detailed information about an Overleaf project including its file tree (folders, docs, and files).', {
    project_id: z.string().describe('The Overleaf project ID (24-char hex string)'),
}, async ({ project_id }) => wrapTool(async () => {
    const client = await getClient();
...
// Tool: pull_project
// ---------------------------------------------------------------------------
server.tool('pull_project', 'Download and extract an Overleaf project as a zip to a local directory. Creates the directory if it does not exist.', {
...
}, async ({ project_id, output_dir, overwrite }) => wrapTool(async () => {
    const client = await getClient();
...
// Tool: push_file
// ---------------------------------------------------------------------------
server.tool('push_file', 'Upload a local file to an Overleaf project. The remote path is inferred from the file_path argument.', {
    project_id: z.string().describe('The Overleaf project ID'),
    local_path: z.string().describe('Absolute or relative path of the local file to upload'),
    remote_path: z
        .string()
        .optional()
        .describe('Target path within the project. Defaults to the basename for absolute local paths, or the relative path as given (e.g. "figures/diagram.png")'),
}, async ({ project_id, local_path, remote_path }) => wrapTool(async () => {
    const client = await getClient();
...
// Tool: compile
// ---------------------------------------------------------------------------
server.tool('compile', 'Compile an Overleaf project using the remote LaTeX compiler. Returns the PDF URL and any log messages.', {
    project_id: z.string().describe('The Overleaf project ID'),
    resource_path: z
        .string()
        .optional()
        .describe('Path of a .tex file in the project to compile as the root document. E.g. "paper.tex", "folder/test.tex"'),
}, async ({ project_id, resource_path }) => wrapTool(async () => {
    const client = await getClient();
...
// Tool: download_pdf
// ---------------------------------------------------------------------------
server.tool('download_pdf', 'Compile an Overleaf project and download the resulting PDF to a local file.', {
    project_id: z.string().describe('The Overleaf project ID'),
    ...
    resource_path: z
        ...
        .optional()
        .describe('Path of a .tex file in the project to compile as the root document. E.g. "paper.tex", "folder/test.tex"'),
}, async ({ project_id, output_path, resource_path }) => wrapTool(async () => {
    const client = await getClient();
...
// Tool: list_comments
// ---------------------------------------------------------------------------
server.tool('list_comments', 'List review comments on an Overleaf project. Supports filtering by status (all, open, resolved).', {
    project_id: z.string().describe('The Overleaf project ID'),
    status: z
...
}, async ({ project_id, status, include_context }) => wrapTool(async () => {
    const client = await getClient();
...
// Tool: get_entities
// ---------------------------------------------------------------------------
server.tool('get_entities', 'Get a flat list of all files and documents in an Overleaf project with their paths and types.', {
    project_id: z.string().describe('The Overleaf project ID'),
}, async ({ project_id }) => wrapTool(async () => {
    const client = await getClient();
...
// Tool: download_file
// ---------------------------------------------------------------------------
server.tool('download_file', 'Download a single file from an Overleaf project by its path within the project.', {
    project_id: z.string().describe('The Overleaf project ID'),
...
}, async ({ project_id, remote_path, output_path }) => wrapTool(async () => {
    const client = await getClient();
...
// Tool: add_comment
// ---------------------------------------------------------------------------
server.tool('add_comment', 'Add a review comment to a specific location in an Overleaf project document.', {
    project_id: z.string().describe('The Overleaf project ID'),
...
}, async ({ project_id, doc_path, content, position, length, selected_text, line, column }) => wrapTool(async () => {
    const client = await getClient();
...
// ---------------------------------------------------------------------------
server.tool('reply_to_comment', 'Add a reply to an existing comment thread on an Overleaf project.', {
    project_id: z.string().describe('The Overleaf project ID'),
    thread_id: z.string().describe('The comment thread ID (from list_comments)'),
    content: z.string().describe('The reply message text'),
}, async ({ project_id, thread_id, content }) => wrapTool(async () => {
    const client = await getClient();
...
// ---------------------------------------------------------------------------
server.tool('resolve_comment', 'Mark a review comment thread as resolved.', {
    project_id: z.string().describe('The Overleaf project ID'),
    thread_id: z.string().describe('The comment thread ID (from list_comments)'),
}, async ({ project_id, thread_id }) => wrapTool(async () => {
    const client = await getClient();
...
// Tool: delete_entity
// ---------------------------------------------------------------------------
server.tool('delete_entity', 'Delete a file or document from an Overleaf project by its remote path.', {
    project_id: z.string().describe('The Overleaf project ID'),
    remote_path: z.string().describe('Path of the file/document to delete within the project'),
}, async ({ project_id, remote_path }) => wrapTool(async () => {
    const client = await getClient();
...
// Tool: rename_entity
// ---------------------------------------------------------------------------
server.tool('rename_entity', 'Rename a file or document within an Overleaf project.', {
    project_id: z.string().describe('The Overleaf project ID'),
...
}, async ({ project_id, remote_path, new_name }) => wrapTool(async () => {
    const client = await getClient();
...
// ---------------------------------------------------------------------------
server.tool('compile_with_outputs', 'Compile an Overleaf project and return all output file metadata (PDF, BBL, logs, etc.). Useful for arXiv submission workflows.', {
    project_id: z.string().describe('The Overleaf project ID'),
    resource_path: z
        .string()
        .optional()
